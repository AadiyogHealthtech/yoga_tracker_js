...
import { checkBendback, getScaledIdealKeypoint, getScalingFactor } from './holding.js';
/**
 * BasePhase
...
export class StartPhase extends BasePhase {
    ...
    process(currentTime) {
        ...
        this.controller.scalingFactor = getScalingFactor(ctx, 12, idealStartingKeypoints, this.controller.normalizedKeypoints, this.controller.hipPoint);
    ...
    if (nk) {
      ...
      if (this._pointQueue.length > 0) {
        ...
        //   const pt = frame[LEFT_WRIST] || frame;
          ...
          const{pix: pt} = getScaledIdealKeypoint(ctx, LEFT_WRIST, frame, this.controller.normalizedKeypoints, this.controller.hipPoint, this.controller.scalingFactor);
          if (!pt) continue;
        ...
        const [ux, uy] = this._toPixelCoords(userRel, ctx.canvas.width, ctx.canvas.height);
...
export class HoldingPhase extends BasePhase {
    ...
    process(currentTime) {
        ...
        if (this.controller.normalizedKeypoints) {
            ...
            const [ctx, success] = checkBendback(this.controller.frame, idealKeypoints, this.controller.normalizedKeypoints, this.controller.hipPoint, this.thresholds, this.controller.scalingFactor);
