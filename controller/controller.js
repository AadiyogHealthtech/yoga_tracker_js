...
export class Controller {
  ...
  constructor(exercisePlan) {
    ...
    this.transitionAnalyzer = new TransitionAnalyzer(
      this.jsonPath,
      this.currentExercise
    );

    //Constructing audio parameters

    this.audioMapping = {
      phase_starting: "START_001", // Your Strapi errorId for starting phase
      phase_transition: "TRANSITION_001", // Your Strapi errorId for transition
      phase_holding: "HOLD_001", // Your Strapi errorId for holding phase
      phase_ending: "END_001", // Your Strapi errorId for ending phase
      phase_relaxation: "REST_001", // Your Strapi errorId for rest mode
      rep_completed: "REP_COMPLETE_001", // Rep completion sound
      exercise_complete: "EXERCISE_DONE_001", // Exercise completion
      workout_complete: "WORKOUT_DONE_001", // Workout completion
    };
    // Initialize Audio Manager
    this.audioManager = new AudioManager(
      "http://your-strapi-url.com", // Replace with your Strapi URL
      this.audioMapping
    );

    // Track last phase to avoid duplicate audio
    this.lastPlayedPhase = null;
  }
  /**
  ...
  _initializeHandlers() {
    ...
    this.segments.forEach((segment, i) => {
      ...
      const { phase, type: phaseType, facing: startFacing } = segment;
      ...
      if (phaseType === "starting") {
        ...
        handlers[uniqueKey] = new TransitionPhase(
          this,
          startFacing,
          this.transitionAnalyzer
        ); // Pass facing
      } else if (phaseType === "holding") {
        // HoldingPhase receives threshold parameters to determine hold duration

        handlers[uniqueKey] = new HoldingPhase(
          this,
          segment.thresholds,
          startFacing
        ); // Add facing
        ...
        handlers[uniqueKey] = new RelaxationPhase(this);
  ...
  update_phase_handlers_frame() {
    ...
    for (const handlerKey of Object.keys(this.phaseHandlers)) {
      ...
      this.phaseHandlers[handlerKey].normalizedKeypoints =
  ...
  updateFrame(results) {
    ...
    if (allVisible) {
      ...
      [this.normalizedKeypoints, this.hipPoint] = normalizeKeypoints(
        this.landmarks
      );
      this.update_phase_handlers_frame();
    } else {
  ...
  checkPhaseTimeouts(currentTime) {
    ...
    if (currentSegment.type === "holding") {
      ...
      if (
        currentTime - this.lastValidHoldTime >
        this.phaseTimeouts.holdingAbandonment
      ) {
  ...
  handleRepCompletion(currentTime) {
    ...
    if (this.count >= this.targetReps) {
      ...
      if (this.currentExerciseIdx < this.exerciseNames.length - 1) {
        // 🎵 AUDIO: Exercise completed, next one starting
        this.audioManager.playPhaseAudio("exercise_complete");
        ...
        // 🎵 AUDIO: Entire workout completed!
        this.audioManager.playPhaseAudio("workout_complete");
  ...
  shouldEnterRelaxation(currentTime) {
    ...
    if (["starting", "ending"].includes(current?.type)) {
      ...
      if (this.landmarks) {
        ...
        console.log(
          `Normalized keypoints testing for facing=${this.normalizedKeypoints}`
        );
        ...
        if (facing != null && facing !== target) {
          console.log(
            `Detected facing=${facing}, expected=${target} → relaxing`
          );
          return true;
        }
      }
  ...
  processExercise(currentTime) {
    ...
    // 🎵 AUDIO TRIGGER: Play audio when entering new phase
    const currentPhaseAudio = `phase_${currentSegment.type}`;
    if (this.lastPlayedPhase !== currentPhaseAudio) {
      this.audioManager.playPhaseAudio(currentPhaseAudio);
      this.lastPlayedPhase = currentPhaseAudio;
    }
    ...
    if (currentSegment.type === "holding" && this.normalizedKeypoints) {
      ...
      if (idealKeypoints.length) {
        const dtwResult = calculateDtwScore(
          idealKeypoints,
          this.normalizedKeypoints
        );
    ...
    if (completed) {
      if (currentSegment.type === "starting") {
        ...
        // Transition phase starting chime
        this.audioManager.playPhaseAudio("phase_transition");
        ...
        if (currentTime - this.startTime > 10000) {
          ...
          // 🎵 AUDIO: Back to relaxation
          this.audioManager.playPhaseAudio("phase_relaxation");
        } else {
          this.currentSegmentIdx++; // Proceed to holding
          this.startTime = currentTime;
          // 🎵 AUDIO: Holding phase starting
          this.audioManager.playPhaseAudio("phase_holding");
        }
        ...
        if (nextSeg.type === "holding") {
          ...
          // 🎵 AUDIO: Next holding phase
          this.audioManager.playPhaseAudio("phase_holding");
        } else if (nextSeg.type === "ending") {
          // 🎵 AUDIO: Ending phase
          this.audioManager.playPhaseAudio("phase_ending");
        }
      } else if (currentSegment.type === "ending") {
        // 🎵 AUDIO: Rep completed
        this.audioManager.playPhaseAudio("rep_completed");
  ...
  getIdealKeypoints(phase) {
    ...
    if (segment.phase === phase) {
      ...
      console.log(
        "New representative frme is : ",
        this.yoga.getIdealKeypoints(
          representativeFrame,
  ...
  getTransitionKeypoints(startIdx, endIdx) {
    for (let i = startIdx; i < endIdx; i++) {
      if (this.segments[i].type === "transition") {
        return this.yoga.getIdealKeypoints(
